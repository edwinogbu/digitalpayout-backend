...
//         const insertQuery = `
//             INSERT INTO users 
//                 (firstName, lastName, email, phone, password, emailVerified, status, role, userType) 
//             VALUES 
//                 (?, ?, ?, ?, ?, ?, ?, ?, ?)
//         `;
//         const result = await query(insertQuery, [firstName, lastName, email, phone, hashedPassword, false, 'pending', role, userType]);
...
authenticationService.getProfile = async (userId) => {
    try {
        ...
        const selectQuery = 'SELECT id, firstName, lastName, email, phone, emailVerified, status, role, userType, defaultEmail, pinCode, createdAt, updatedAt FROM users WHERE id = ?';
